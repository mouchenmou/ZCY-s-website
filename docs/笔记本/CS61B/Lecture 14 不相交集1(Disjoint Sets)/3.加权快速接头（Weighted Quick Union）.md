...
- Inductive step：当 $N=k+1$ 的时候，这棵树可以看成是两棵树合并而成的，$T_1$ 的节点数为 $N_1$，$T_2$ 的节点数为 $N_2$，令 $N_1>N_2，\ N1+N2=k+1$，最坏的情况就是 $T_1$ 的树的高度是 $log_{2}N_1+1$，$T_2$ 的高度为 $log_{2}N_2+1$ ，$T$ 的高度$h \ \le max(height(T_1), \ height(T_2)+1)\ = max(log_{2}N_1+1, \ log_{2}N_2+2)$
    - 由于 $N_1<k+1$，所以 $log_{2}N_1+1\ \le log_{2}(k+1)+1$
    - 由于$N_2< \frac{k+1}{2}$，所以$log_{2}N_2+2 \ <\ log_{2}(k+1)-1+2\ =\ log_{2}(k+1)+1$
