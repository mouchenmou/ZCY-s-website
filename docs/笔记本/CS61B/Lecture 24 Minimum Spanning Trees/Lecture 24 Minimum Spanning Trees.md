...
- **为什么？** 因为 Kruskal 的死规矩是**边长从小到大排好序，挨个看**！
    
    如果从这个 `{0, 1, 7}` 的圈子，连向外部世界有一条**比 `0-2` 更短**的路，那 Kruskal 怎么可能今天才看到 `0-2`？它**早就**应该先看到那条更短的路，并且把它变成黑色的了！
    
    既然目前黑边都缩在圈里，说明在这之前根本没修通向外面的路。此刻轮到了合法的 `0-2`，那它绝对是我们遇到的**第一条**通向外面的路，也就是 **最短** 的跨界边。
