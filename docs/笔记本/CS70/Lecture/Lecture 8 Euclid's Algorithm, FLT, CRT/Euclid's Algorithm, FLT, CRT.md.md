...
!!! tip "为什么会想到取 $k$ 次方？"
    ...
    也就是说，$k$ 就是 $7$ 在 mod 10 下的乘法逆元，它在指数运算中扮演了"$1/7$"的角色。
...
!!! warning "前提条件：我的疑问：万一$e$ 和 $p-1$ 不互质怎么办？"
    ...
    这个方法有个关键前提：$e$ 在 mod $(p-1)$ 下必须存在乘法逆元，即 $\gcd(e, p-1) = 1$。

    如果 $\gcd(e, p-1) \neq 1$，那么 $e$ 在 mod $(p-1)$ 下没有逆元，这个"取 $d$ 次方"的方法就行不通了。

    **为什么 RSA 不会遇到这个问题？**

    RSA 在选公钥 $e$ 的时候就强制要求 $\gcd(e, (p-1)(q-1)) = 1$，所以逆元 $d$ 一定存在。但如果只是随便拿来一个 $e$，这个条件不一定满足。
