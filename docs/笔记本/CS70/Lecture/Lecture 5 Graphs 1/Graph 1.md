...
- 度数: 与该岛屿相连的桥的数量（在简单图中，桥的数量就是邻居的数量）。
    - 图的度数 $deg(G)$：图中所有的顶点的度数之和，
        - 引理：图$G\ =\ (V, \ E)$ 中，边的数量为这个图的度数除以2，即 $|E|\ =\frac{\sum_{v \in V}^{} deg(v)}{2}$
![](附件/Pasted%20image%2020260419181452.png)
### 1.2 Directed Graph

设 $V$ 为图 $G$ 的顶点集。那么有向边的集合 $E$ 就是 $V \times V$ 的子集，即 $E \subseteq V \times V$。（$U \times V$ 表示集合 $U$ 和 $V$ 的笛卡尔积，定义为 $U \times V = \{(u, v) : u \in U \text{ 且 } v \in V\}$。）例如，如果 $V = \{1, 2, 3, 4\}$ 且 $E = \{(1, 2), (1, 3), (1, 4)\}$，那么对应的图就是下图中所示的 $G_1$。

![](附件/Pasted%20image%2020260426180540.png)
### 注意：

$G_1$ 中的每条边都有一个由箭头指定的方向。例如，$(1, 2) \in E$ 但 $(2, 1) \notin E$。有向图在建模单向关系时非常有用，比如两个地点之间的单行道。另一方面，如果每条边都是双向的，即 $(u, v) \in E$ 当且仅当 $(v, u) \in E$，那么我们称这个图为无向图。对于无向图，我们舍弃了边的有序对表示法，简单地用集合 $\{u, v\}$ 来表示 $u$ 和 $v$ 之间的边。无向图自然地模拟了诸如地点之间的双向街道这类关系；图 2 中的 $G_2$ 给出了一个例子。为了简单起见，我们在画无向图的边时省略了箭头。无论哪种情况，一个图都可以正式地由一个有序对 $G = (V, E)$ 来指定，其中 $V$ 是顶点集，$E$ 是边集（有向或无向）。
