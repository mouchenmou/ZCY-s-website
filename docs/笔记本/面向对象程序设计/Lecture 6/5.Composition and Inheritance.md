...
这张对象图展示了媒体收藏软件在运行时内存中的数据结构：

1. 程序中有一个唯一的数据库对象(Database)。
2. 这个数据库对象通过其内部的两个列表（cds和dvds）来管理所有的媒体项目。
3. cds列表持有所有具体的CD记录
4. dvds列表中持有所有具体的DVD记录

换句话说，这幅图展示了一个Database拥有一个CD列表和一个DVD列表，而这些列表又拥有许多具体的媒体对象运行时状态。

## Source code
### 下面是用java写出的source code
...
!!! source "1. 存储结构"
    ...
    - `Database`类：这是媒体数据库的容器类。
    - `cds`和`dvds`：这两个是`Database`类的私有属性（即只有`Database`内部才能直接访问到）
        - 它们都是`Arraylist`类型，这是一种动态数组或列表结构。
        - `cds`专门用于存储`CD`类型的对象
        - `dvds`专门用于存储`DVD`类型的对象
    - 作用：这些列表是`Database`用来长期存放和管理所有`CD`和`DVD`记录的地方。
...
!!! source "2. 添加数据的方法"
    ...
    - `addCD ( CD theCD)`：一个公共方法，允许外部程序传入一个`CD`对象。它调用`cds`列表的`add()`方法，将该`CD`对象加入到列表中。
    - `addDVD( DVD theDVD )`：一个公共方法，允许外部程序传入一个 `DVD` 对象。它将该 `DVD` 对象加入到 `dvds` 列表中。
    - 作用：提供了向数据库中安全添加新媒体项目的接口。
...
!!! source "3. 列出数据的方法"
    ...
    - `list`()：一个公共方法，用于显示所有存储的媒体信息
    - `for(CD cd : cds)`：遍历`cds`列表中每个`CD`对象
    - 多态调用(`cd.print()`)：调用当前`CD`对象的`print()`方法。这个方法负责输出该`CD`自身的详细信息
    - `System.out.println()`：这是`java`的标准输出语句，用于打印一个项目后进行换行
---
...
!!! tips "What if we add new types of media?"
    ...
    ![](附件/Pasted%20image%2020251108154217.png)
