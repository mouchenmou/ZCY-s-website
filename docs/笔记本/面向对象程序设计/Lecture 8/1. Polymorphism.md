...
!!! conception "### Shape"
    ...
    以shape为例来说明虚函数是如何做到打印对象的实际类型而非被指带类型的。
    ```c++
        shape *p = circ;
        p->render();
    ```
    如果`shape`中并没有定义虚函数，那么此时输出的应该是 `"Shape render is called"`。因为p是`Shape`类型的指针，只不过它的地址是`circ`的地址，并且这个地址中也有`render`函数，所以会打印出`Shape`的`render`函数。
    
    如果`Shape`中定义了虚函数，那么此时输出的应该是` "Circle render is called"`。此时虽然p是`Shape`类型的指针，但是`Shape`中并没有`render`这个函数，而只有`vptr`指针，此时p指向的值是`circ`的地址，因此调用`render`函数的时候，会进入`circ`的`vtable`中，因此打印的是`circle`的`render`函数。
!!! conception "Ellipse"
    ### Ellipse
...
- Vptr from circ is ignored
    - the vptr in elly still point to the Ellipse vtable (elly 中的 vptr 仍然指向 Ellipse 的 Vtable)
    - 解释：赋值操作 `elly = circ;` 只是数据拷贝，它不会改变目标对象 `elly` 的类型或 `VTable`指针。
...
- `elly` 最初指向的那个 `new Ellipse`对象，现在没有任何指针指向它了。
- 指针`elly`和`circ`现在都存储着同一个内存地址，即那个通过`new Circle`创建的`Circle`对象的地址。
