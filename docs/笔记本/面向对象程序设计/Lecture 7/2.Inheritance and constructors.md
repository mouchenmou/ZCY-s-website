...
!!! answer "方法二"
    ...
    ```c++
        ...
        ss ->get_title();
    ```
    ![](附件/Pasted%20image%2020251120204903.png)
    ![](附件/Pasted%20image%2020251120205001.png)
...
1. Upcasting is the act of converting from a derived reference or pointer to a base class reference or pointer.
    - **Up-casting** 是指将一个派生类的引用或指针，转换成一个基类的引用或指针的行为。
    - 在类继承的层次结构中，派生类位于基类的下方（如下图所示），所以从派生类向基类转换，就被形象地称为向上转型。
        - ![](附件/Pasted%20image%2020251120162920.png)
2. Is to regard an object of the derived class as an object of the base class.
    - Up-casting 就是将一个派生类的对象，视作一个基类的对象。
    - 这强调的是视角和使用方式的转变，而不是对象本身的改变。对象本质上还是派生类对象，只是我们现在只能通过基类提供的接口来访问它。
3. It is to say: Students are human beings. You are students. So you are human being.
    - 这个例子完美地体现了面向对象编程中的“Is-A”关系，这是继承的基础：
        - 基类： Human being
        - 派生类： Students
            - ![](附件/Pasted%20image%2020251120163750.png)

!!! example "up-casting example"
    ```c++
        Manager pete("Pete", "444-55-6666", "Bakery");
        Employee *ep = &pete; //upcast
        Employee &es = pete; //upcast
        ep->print(cout); //调用的就是Employee的print 
    ```