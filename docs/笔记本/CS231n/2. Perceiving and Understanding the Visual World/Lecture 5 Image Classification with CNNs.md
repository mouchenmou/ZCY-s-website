...
2. **参数数量增长得太快**。一张 $200\times200\times3$ 的图片有 $120000$ 个输入值，每一个全连接神经元就都需要 $120000$ 个权重。如果隐藏层有很多神经元，参数量会立刻变得非常大。
...
!!! explanation "我的疑惑1"
    ...
    上面的“卷积核 1 学横向边缘、卷积核 2 学竖直边缘”只是训练完成后可能出现的结果。我们在训练开始前并不知道每个卷积核最终会学习什么，也不会手动规定你必须学习横向边缘。网络只知道最终目标是降低分类的 loss。
...
!!! explanation "引入 Stride 的原因"
    ### 我的疑问
    明明引入Zero Padding就是为了防止输出变小，那为什么又要引入 Stride 呢？这不是又让输出变小了吗？
    
    ### 解答
    这两个操作看起来互相矛盾，是因为它们都会影响输出尺寸，但它们的目的完全不同：
    
    1. **Zero Padding** 防止特征图在每一层卷积时发生没有计划的缩小，并避免图片边缘的信息过快消失。
    2. **Stride** 在网络确实需要缩小特征图时，主动进行有计划的下采样。
    
    所以，Padding 不是要求特征图从头到尾永远不能变小，而是让我们能够控制它什么时候变小。
    
    ### 为什么不能让所有特征图一直保持原来大小？
    
    假设输入图片为 $224\times224$。如果几十层卷积始终使用 $S=1$ 和 same padding，那么每一层都会保持 $224\times224$。
    
    这样会产生三个问题。
    
    1. **计算量非常大**：后面的每一层都要在 $224\times224$ 个位置上进行卷积。
    2. **占用大量内存**：反向传播时需要保存很多大尺寸 activation map。
    3. **后层神经元很难快速看到大范围信息**：空间尺寸一直很大时，需要堆叠很多层，感受野才能覆盖图片中的大范围区域。
    
    因此，CNN 通常不会让特征图永远保持原始大小，而是采用下面的节奏：
    
    1. 先使用若干个 $S=1$、带 padding 的卷积层，在当前分辨率下充分提取特征。
    2. 再偶尔使用一个 $S=2$ 的卷积层或 pooling layer，将高度和宽度缩小一半。
    3. 在更小的特征图上继续学习更抽象的特征。
    ...
    前面的高分辨率特征图适合记录边缘的准确位置；后面的低分辨率特征图不再关心每个像素的精确位置，而更关心“这里大致有没有眼睛、车轮或某种纹理”。
    
    ### Padding 和 Stride 可以同时使用
    ...
    这时 padding 抵消了卷积核带来的边缘缩小，输出与输入一样大。
    
    当 $S=2$ 时：
    
    $$
    W'=\left\lfloor\frac{W-3+2}{2}\right\rfloor+1
    \approx\frac{W}{2}
    $$
    
    这时 padding 仍然让卷积核能够照顾输入边缘，而 stride 负责有计划地把空间尺寸缩小约一半。二者并不冲突。
    
    ### 下采样会不会丢失信息？
    
    会。Stride 越大，跳过的空间位置越多，丢失的细节也越多，所以不能在每一层都随意使用很大的 stride。
    
    CNN 的做法是先用卷积把原始像素转换成更有意义的特征，再在适当位置下采样。这样虽然牺牲了一部分精确位置信息，却换来了：
    ...
    - **不加 Padding 导致的缩小**：每做一次普通卷积都被动地损失边缘和尺寸。
    - **使用 Stride 导致的缩小**：在选定的层主动下采样，是网络结构的一部分。
...
!!! explanation " $\frac{W-K+2P}{S}+1$ 是什么"
    
    $\frac{W-K+2P}{S}$ 计算的是：==卷积核从初始位置出发，最多可以移动多少步==；但是输出尺寸要计算的是：==卷积核总共可以放置多少次==。
    
    初始位置还没有移动，却已经能进行一次卷积，所以放置次数会比移动次数多 $1$。
    
    假设：
    
    $$
    W=7,\qquad K=3,\qquad P=0,\qquad S=2
    $$
...
$$
\frac{W-K+2P}{S}=\frac{10-3}{2}=3.5
$$

它不是整数，所以卷积核无法在最后一步刚好对齐输入末端，不能直接得到整数输出尺寸。
...
!!! explanation "这些 shape 中的 $N,C_{in},C_{out},H,W,K_H,K_W$ 分别代表什么？"
    ### 1. 输入张量 $X$
    ...
    1. $N$：一个 batch 中图片的数量，也叫 **batch size**。它不是整个数据集的图片总数，而是本次一起送进网络的图片数量。
    2. $C_{in}$：每张输入图片或输入特征图的通道数，也可以理解成输入的“厚度”。
    3. $H$：每个通道的高度，即 **Height**。
    4. $W$：每个通道的宽度，即 **Width**。
    
    如果一次输入 $8$ 张 RGB 图片，每张图片大小为 $32\times32$，那么：
    
    $$
    X\in\mathbb{R}^{8\times3\times32\times32}
    $$
    
    其中：
    
    1. $N=8$：这一次输入 $8$ 张图片。
    2. $C_{in}=3$：每张图片有 R、G、B 三个颜色通道。
    3. $H=32$：图片高度为 $32$。
    4. $W=32$：图片宽度为 $32$。
    
    对网络第一层来说，$C_{in}=3$ 通常对应 RGB 三个颜色通道。但是在更深的卷积层中，输入不再是原始图片，而是上一层输出的 feature maps。例如上一层输出 $64$ 张 feature maps，那么下一层就有：
    
    $$
    C_{in}=64
    $$
    
    所以把 $C_{in}$ 理解成“输入层的厚度”是可以的，但要记住这个厚度表示的是通道数，不是神经网络一共有多少层。
    ...
    1. $C_{out}$：这一层拥有的卷积核数量，同时也是输出通道数。
    2. $C_{in}$：每个卷积核的深度。它必须等于输入张量的通道数。
    3. $K_H$：每个卷积核的高度，即 **Kernel Height**。
    4. $K_W$：每个卷积核的宽度，即 **Kernel Width**。
    
    因此，你说的这句话是正确的：
    
    $$
    \underbrace{C_{in}\times K_H\times K_W}_{\text{一个卷积核的完整尺寸}}
    $$
    
    最前面的 $C_{out}$ 则表示一共有多少个这种尺寸的卷积核。所以完整权重张量可以理解成：
    
    $$
    \underbrace{C_{out}}_{\text{卷积核数量}}
    \times
    \underbrace{\left(C_{in}\times K_H\times K_W\right)}_{\text{单个卷积核的尺寸}}
    $$
    
    例如：
    
    $$
    W\in\mathbb{R}^{10\times3\times5\times5}
    $$
    
    表示这一层有 $10$ 个卷积核，每一个卷积核的尺寸都是：
    ...
    $$
    \underbrace{10}_{\text{一共有 10 个卷积核}}
    \times
    \underbrace{\left(3\times5\times5\right)}_{\text{每个卷积核包含 3 个通道，每个通道是 }5\times5}
    $$
    
    这里有一个容易混淆的地方：公式中的 $W$ 既可能表示图片的宽度，也经常被用来表示权重张量。必须根据上下文判断：
    
    1. 在 $N\times C_{in}\times H\times W$ 中，$W$ 是图片或特征图的宽度。
    2. 在“权重 $W\in\mathbb{R}^{C_{out}\times C_{in}\times K_H\times K_W}$”中，最前面的 $W$ 是整个权重张量的名字。
    ...
    每个卷积核都有一个自己的偏置。因为一共有 $C_{out}$ 个卷积核，所以一共有 $C_{out}$ 个偏置。
    ...
    1. $N$：输出仍然对应原来的 $N$ 张输入图片，所以 batch size 不变。
    2. $C_{out}$：每个卷积核产生一张 activation map，因此输出通道数等于卷积核数量。
    3. $H'$：经过 kernel、padding 和 stride 计算后得到的输出高度。
    4. $W'$：经过 kernel、padding 和 stride 计算后得到的输出宽度。
    ...
    $$
    Y\in\mathbb{R}^{8\times10\times32\times32}
    $$
    
    可以把整个过程概括成：
    
    $$
    \underbrace{8\times3\times32\times32}_{\text{8 张、每张 3 个输入通道}}
    \xrightarrow[\text{10 个卷积核}]{10\times3\times5\times5}
    \underbrace{8\times10\times32\times32}_{\text{8 张、每张 10 个输出通道}}
    $$
